...
type ClosestPhilosopher struct {
	Philosopher *domain.Philosopher `json:"philosopher"`
	Distance    float64            `json:"distance"`
}

// FindClosestPhilosopher ユーザーの回答に最も近い哲学者を検索
func FindClosestPhilosopher(userAnswer *domain.Answer, philosophers []domain.Philosopher) *ClosestPhilosopher {
	if len(philosophers) == 0 {
		return nil
	}

	userVector := userAnswer.ToVector()
	var closest *domain.Philosopher
...
func CalculateEuclideanDistance(v1, v2 domain.AnswerVector) float64 {
	var sum float64
	for i := 0; i < 16; i++ {
